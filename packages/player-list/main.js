...
var playertable = playerListTable = document.createElement("table"),
    windo = playerListWindow = OWOP.windowSys.addWindow(new OWOP.windowSys.class.window("Player list", {
        closeable: true
    }, win => {
        var t = document.createElement("tr");
        t.innerHTML = "<th>Id</th><th>X</th><th>Y</th><th>Color</th>";
        playertable.appendChild(t);
        win.container.appendChild(playertable);
        win.container.id = "player-list";
    })).move(window.innerWidth - 240, 32);

let bot = new OJS.Client({reconnect: true, controller: true});

bot.on("join", () => {
    setTimeout(() => {
        setInterval(() => {
            for(let i in bot.players) {
                delete bot.players[bot.player.id];
                let player = bot.players[i];
                try {
                    let a = document.getElementById("player-"+player.id);
                    if(!a) {
                        var d = document.createElement("tr");
                        d.innerHTML = `<td>`+player.id+`</td><td id="player-${player.id}-x">`+player.x+`</td><td id="player-${player.id}-y">`+player.y+`</td><td style="color: rgba(${player.rgb.toString()}")>■</td>`;
                        d.id = 'player-' + player.id;
                        playerListTable.appendChild(d);
                    } else if(a && bot.players[i]){
                        let d = document.getElementById("player-"+player.id);
                        d.innerHTML = `<td>`+player.id+`</td><td id="player-${player.id}-x">`+player.x+`</td><td id="player-${player.id}-y">`+player.y+`</td><td style="color: rgba(${player.rgb.toString()}")>■</td><button style="width: 12px;height: 18px;" id="player-${player.id}-tp"></button>`;
                        tpbtn = document.getElementById(`player-${player.id}-tp`);
                        tpbtn.addEventListener("mousemove", () => {OWOP.camera.teleport(player.x, player.y)});
                    };
